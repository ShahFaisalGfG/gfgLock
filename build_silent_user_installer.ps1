<#
.SYNOPSIS
    Build the gfgLock silent per-user Windows installer.
.DESCRIPTION
    Step 1 — PyInstaller  : bundles the app into dist\gfgLock\
    Step 2 — Inno Setup   : compiles the silent per-user installer into build\installer\
.NOTES
...
$IssFile   = "installer\gfglock_silent_user_installer.iss"
$OutputExe = "build\installer\${AppName}_${Version}_silent_user_installer.exe"
$IsccPaths = @(
