...
from utils.gfg_helpers import load_settings, write_general_log, write_critical_log, write_log
...
class EncryptDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, mode="encrypt"):
        ...
        chunks = [
            ...
            # should I add 128 MB profile or not?
            ("128 MB (expiremental)", 128*1024*1024),
        ]
        ...
        if self.mode == "encrypt":
            ...
            self.alg_combo.addItem("AES-256 GCM (Recommended - AEAD)", "aes256_gcm")
            self.alg_combo.addItem("AES-256 CFB (Fast - No AEAD)", "aes256_cfb")
            self.alg_combo.addItem("ChaCha20-Poly1305 (AEAD)", "chacha20_poly1305")
            try:
